...
class softmax_weights(lasagne.layers.Layer):
    def __init__(self, num_components, incoming, **kwargs):
        super(softmax_weights, self).__init__(incoming, **kwargs)
        #num_inputs = self.input_shape[1]
        self.num_components = num_components
        initial_value = lasagne.init.Constant(1./self.num_components)
        self.logits = self.add_param(initial_value, (1,self.num_components), name='logits')

    def get_output_for(self, incoming,  **kwargs):
        return T.nnet.softmax(self.logits) + incoming

    def get_output_shape_for(self):
        return (1,self.num_components)
